...
from yacht_ai.policies.advice import (
    build_reason,
...
from yacht_ai.value.learned import load_linear_value_model
    ...
    for cat_name in hand_cats_display:
        ...
        if len(tie_keeps) > 1:
            ...
            if cat_name in ("Small Straight", "Large Straight"):
                ...
                display = sorted(
                    [values for values, length in unique_values.items() if length == min_len],
                    reverse=True,
                )[:1]
