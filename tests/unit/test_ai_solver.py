...
from yacht_engine import solve_best_move
...
class AiSolverRecommendationTests(unittest.TestCase):
    ...
    def test_score_stage_positive_comparison_rows_are_not_labeled_sacrifice(self):
        scorecard = [None, None, None, None, None, 18, 20, 25, 15, 15, 30, 0]
        open_categories = [idx for idx, value in enumerate(scorecard) if value is None]
        ...
        positive_rows = [row for row in result["breakdown"] if row.get("val_str") != "0점"]
        self.assertTrue(positive_rows)
        self.assertFalse(any(row.get("type") == "sacrifice" for row in positive_rows))
        self.assertFalse(any("희생" in row.get("keep_str", "") for row in positive_rows))
