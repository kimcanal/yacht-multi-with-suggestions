...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="stream",
        help="stream preserves legacy RNG consumption; indexed fixes dice slots for fair paired comparisons",
    )
    parser.add_argument(
        "--policy-set",
        choices=("all", "second"),
        default="all",
        help="all compares exact + both models; second compares exact + --model-v2 only",
    )
...
def play_game(seed: int, mode: str, policy: dict, random_source: str) -> dict:
...
def summarize_policy(label: str, games: list[dict], exact_games: list[dict] | None = None) -> dict:
    ...
    return {
        ...
        "wins_vs_exact": sum(1 for delta in paired_delta if delta > 0),
        "ties_vs_exact": sum(1 for delta in paired_delta if delta == 0),
        "losses_vs_exact": sum(1 for delta in paired_delta if delta < 0),
        "win_rate_vs_exact": round(
            sum(1 for delta in paired_delta if delta > 0) / len(paired_delta), 6
        ) if paired_delta else 0.0,
        "non_loss_rate_vs_exact": round(
            sum(1 for delta in paired_delta if delta >= 0) / len(paired_delta), 6
        ) if paired_delta else 0.0,
...
def main() -> None:
    ...
    policies = (
        all_policies
        if args.policy_set == "all"
        else [all_policies[0], all_policies[2], all_policies[4]]
    )
    ...
    for label, policy in policies:
        ...
        for idx, seed in enumerate(seeds, start=1):
            games.append(play_game(seed, args.mode, policy, args.random_source))
    ...
    for row in summaries:
        print(
            ...
            f"W/T/L={row['wins_vs_exact']}/{row['ties_vs_exact']}/{row['losses_vs_exact']}"
        )
