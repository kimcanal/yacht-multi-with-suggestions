...
- **PC** — 게임을 열면 자동으로 테이블 보기를 사용합니다. 왼쪽 점수판, 가운데 주사위와 ROLL, 오른쪽 AI 코치를 한 화면에 두어 현재 선택과 점수 흐름을 함께 볼 수 있습니다.
...
화면 크기에 따라 게임 화면은 자동으로 전환됩니다. PC에서는 `/game/single/table`, `/game/multi/table` 테이블 보기를 사용하고, 모바일에서는 `/game/single`, `/game/multi` 세로 보기를 사용합니다.
