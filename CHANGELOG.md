...
승률 UI는 30샘플 빠른 추정을 먼저 보여준 뒤 같은 상태를 100샘플로 자동 보정한다. Upper Bonus는 상단 요약에서 `63점 달성 시 +35점`으로 설명하고 점수판 내부의 중복 Subtotal/Bonus 행은 제거했다. 멀티 채팅은 여섯 가지 감정표현으로 교체했으며, 상대 화면에 큰 이모지와 짧은 효과음을 전용 `reaction` SSE event로 전달한다.
